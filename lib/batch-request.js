...
var batchRequest = function( params ) {
    ...
    params.sendResponse = (typeof params.sendResponse === 'undefined') ? true : params.sendResponse;
    params.timeout = (typeof params.timeout === 'undefined') ? 5000 : params.timeout;
    ...
    var _prepareRequest = function( req, r, rp ) {
        ...
        // set the request timeout to slightly longer than promise timeout to handle this gracefully but still kill the request
        if( params.timeout ) {
            r.timeout = params.timeout;
        }
    ...
    var batch = function( req, res, next ) {
        ...
        var recurseDependencies = function( reqs ) {
            ...
            _.each( requestPromises, function( rp, key ) {
                ...
                if( dependent ) {
                    requestPromises[ dependentKey ] = new Promise( function( resolve, reject ) {
                        rp
                            ...
                            .catch( function( err ) {
                                if( [ 'ETIMEDOUT', 'ESOCKETTIMEDOUT' ].indexOf( err.code ) !== -1  ) {
                                    resolve( {
                                        'statusCode': 503,
                                        'body': {},
                                        'headers': {}
        ...
        Promise
            .props( requestPromises )
            .then( function( result ) {
                // remove all properties, except status, body, and headers
                var output = {};
                res.batch_statuses = res.batch_statuses || {};

                for( var prop in result ) {
                    if( !result.hasOwnProperty( prop ) ) continue;

                    // make sure response ends up as JSON if it's supposed to be JSON
                    var content_type = result[ prop ].headers[ 'content-type' ] || result[ prop ].headers[ 'Content-Type' ] || '';
                    if( content_type.search( /json/ ) && _.isString( result[ prop ].body ) ) {
                        try {
                            result[ prop ].body = JSON.parse( result[ prop ].body );
                        } catch( err ) {
                            console.log( 'response is not JSON: ', result[ prop ].body );
                            // just leave as is if it's weird JSON
                        }
                    }
                    ...
                    res.batch_statuses[ prop ] = result[ prop ].statusCode;
                ...
                req.batch_output = output;

                params.postBatch( req, res, next );
                next();
            } )
    ...
    batch.validate = function( req, res, next ) {
        ...
        if( err !== null ) {
            if( params.validateErrorHandler ) {
                return params.validateErrorHandler( err, res, next );
            }
