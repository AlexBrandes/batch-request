...
var batchRequest = function(params) {
    ...
    var _prepareRequest = function( req, r, rp ) {
        ...
        if( rp ) {
            var dependency_parameter_regex = /{([A-Za-z]+)}/g;

            var match = dependency_parameter_regex.exec( r.url );
            while (match != null) {

                var url_pre_replacement = r.url.substring( 0, match.index );
                var url_replacement = rp.body[ match[1] ];
                var url_post_replacement = r.url.substring( match.index + match[0].length );

                r.uri = r.url = url_pre_replacement + url_replacement + url_post_replacement;

                match = dependency_parameter_regex.exec( rp.body );
            }
        }
